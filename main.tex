...
\begin{filecontents*}{moderncvcolorblack.sty}

\definecolor{color0}{rgb}{0.2,0.2,0.345}% dark-blue
%\definecolor{color1}{rgb}{0.2,0.2,0.2}% Testcolor
\definecolor{color1}{rgb}{0.2,0.2,0.345}% dark-blue
\endinput
...
%% Personal data
\firstname{Ignacio}
\familyname{Ferrer Sanz}
\mobile{+34~608~83~44~25}
\email{ignaciofer31@gmail.com}
\homepage{linkedin.com/in/ignacio-ferrer-sanz}
\extrainfo{\github{
\href{https://github.com/Nacchofer31}{github.com/Nacchofer31}
}
}
\photo[64pt][0.4pt]{cvphoto.jpg}
\title{Mobile \& DevOps Developer}
%%------------------------------------------------------------------------------
%% Content
%%------------------------------------------------------------------------------
\begin{document}
...
\cvitem{ }{
\begin{itemize}%
    \item Accomplished Software Developer well-versed in creating native and hybrid applications for Android, iOS and web platforms with good knowledge of various programming languages and frameworks. 
...
\cvitem{May 2020}{ \textbf{Mobile Developer at Cuatroochenta / DEVELAPPS}, \textit{Valencia, Spain.}}
\cvitem{to PRESENT}{
    \begin{itemize}%
    \item Creating cross-platform apps with Flutter. Working with: BLoC pattern, Clean Architecture, RxDart, Dartz, GetIt, Mockito, Isar...
    \item Developing native Android apps with Android (\textit{kotlin}). Working with: Model-View-ViewModel, Jetpack, Retrofit, AndroidX, Glide, Room...
    \end{itemize}
}
\cvitem{Feb 2016}{ \textbf{Junior Android Developer at Sportmaniacs}, \textit{Valencia, Spain.}}
\cvitem{to Sep 2016}{
    \begin{itemize}%
    \item Worked with Model View Presenter, Retrofit, UnitTesting, ButterKnife, RxJava...
    \item Created UI tests with Expresso and Mockito runned in a Jenkins server.
    \end{itemize}
}

\section{Education} % =====================================

\cvitem{2021}{ \textbf{Master's degree in DevOps (Remote)} }
\cvitem{to PRESENT}{
\textit{La Universidad en Internet (UNIR), La Rioja, Spain }
}

\cvitem{2020}{ \textbf{Web Development | Mobile Applications}, \textit{Erasmus+ program, \textit{University of Applied Sciences, Oulu, Finland} }
}

\cvitem{2016 to 2020}{ \textbf{Bachelor's Degree in Informatics Engineering}, \textit{Universidad Politécnica de Valencia (UPV), Valencia, Spain }
}

\cvitem{2014 to 2016}{ \textbf{Higher Technician in Multi-platform Applications Development}, \textit{Florida Universitaria}, Catarroja, Valencia, Spain}
...
\cvitem{ }{
\begin{itemize}
    \item \textbf{Programming Languages}: Kotlin, Dart, Java, C\#, Javascript \& Typescript, SQL, Bash, Ruby, JSON, XML, HTML, Markdown, \LaTeX.
    \item \textbf{Tools and Technologies}: Android SDK, Flutter, NodeJS, Angular, Cordova, Docker, Unit Testing, Restful, Andoid Studio, Visual Studio, XCode.
    \item \textbf{VCS}: Git.
    \item \textbf{Methodologies}: MVVM \& MVP architecture, Test Driven Development and SCRUM.
    \item \textbf{Operating Systems}: Arch Linux, MacOS X, Windows, Raspbian, Android and iOS.
    \item \textbf{CI/CD}: Docker, Ansible, Chef, Puppet, Terraform, Packer, Jenkins, AWS, Azure.
    \item \textbf{Languages}: Spanish (\textit{Native}), English (\textit{Proficient}), Catalan (\textit{Proficient}).
\end{itemize}
}
