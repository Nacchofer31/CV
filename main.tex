...
\cvitem{to Sep 2016}{
    \begin{itemize}%
    \item Part of a team that was tasked with developing an Android mobile application to time races in places that are hard to get to whether there is coverage or not, using a RFID reader connected to the mobile via Bluetooth.
    \item Assisted in developing an Android mobile application to sign up races, genate tickets, browse race results and generate certificates.
...
\cvitem{ }{
\begin{itemize}
    \item \textbf{Programming Languages}: Java, Javascript, Kotlin, SQL, C\#, Swift, RESTful, JSON, XML, markup languages including HTML, R Markdown and \LaTeX.
    \item \textbf{Tools and Technologies}: Android SDK \& NDK, Andoid Studio, Docker, Unit Testing, XCode, Visual Studio, Eclipse and Vim.
    \item \textbf{Operating Systems}: Windows, Arch Linux, MacOSX and Android.
    \item \textbf{VCS}: Git.
    \item \textbf{Methodologies}: Test Driven Development and SCRUM.
    \item \textbf{Operating Systems}: Windows, Arch Linux, iOS and Android.
    \item \textbf{CI}: Jenkins.
    \item \textbf{Languages}: Spanish (\textit{Native}), English (\textit{Upper Intermediate}), Catalan (\textit{Proficient}).
\end{itemize}
}
